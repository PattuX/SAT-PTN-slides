...
\begin{frame}{Cube-and-conquer solving}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item use parallelization (800 cores)
		\end{itemize}
	...
	\begin{figure}
		...
		% edges
		\draw[] (x5) -- node[above] {t} (x21);
		\draw[] (x5) -- node[above] {f} (x7);
		
		\draw[] (x21) -- node[above] {t} (x3);
		\draw[] (x21) -- node[above] {f} (p3);
		
		\draw[] (x3) -- node[above] {t} (p1);
		\draw[] (x3) -- node[above] {f} (p2);
		
		\draw[] (x7) -- node[above] {t} (x22);
		\draw[] (x7) -- node[above] {f} (x6);
		
		\draw[] (x22) -- node[above] {t} (p4);
		\draw[] (x22) -- node[above] {f} (p5);
		
		\draw[] (x6) -- node[above] {t} (p6);
		\draw[] (x6) -- node[above] {f} (x23);
		
		\draw[] (x23) -- node[above] {t} (p7);
		\draw[] (x23) -- node[above] {f} (p8);
		...
		% captions
		\visible<4-3>{\node(look-ahead1) at ($(x5) + (-4.5, 0)$){look-ahead}; }
		\visible<6->{\node[text = OrangeRed1, align=center](3000) at ($(look-ahead1) + (-1, -1.2)$){$\approx$ 3000 \\binary clauses};}
		\visible<5-5>{\node(look-ahead2) at ($(look-ahead1) + (0, -1.5)$){look-ahead};}
		\visible<4->{\node[text = mdwblue, align = center](3450) at ($(look-ahead2) + (-1, -1.25)$){3450 unassigned\\ variables};}
		\visible<5-5>{\node(CDCL) at ($(look-ahead2) + (0, -2)$){CDCL};}
...
\begin{frame}{Validation of the program}
	...
	\begin{itemize}
		\item each step must be validated
	\end{itemize}
	\begin{figure}
		\begin{tikzpicture}
			\node[draw = black, inner sep=30pt] at (0,0) {Picture of the Framework};
		\end{tikzpicture}
	\end{figure}
\end{frame}
